%%%StripesKinetics_Dur_ee.m reads compiledResults_ectopicFlag.mat files,
%%%and plots koff values of endogenous and ectopic nuclei sorted 
...
function StripesKinetics_Dur_ee2
...
for i = 1:length(F)
    ...
    %Save mean duration values (koff) and ste over each fluorescent bin
    koff = 1 ./ compiledResults.dur_vec_mean;
    koff_ste = compiledResults.dur_vec_ste ./ (compiledResults.dur_vec_mean .^ 2);
    
    freq = compiledResults.freq_vec_mean;

    fluo_mean = compiledResults.fluo_mean ./ dAU;
    ee_flag = compiledResults.additionalGroupVec; %endogenous or ectopic flag
    
    maxKoff_vec(i) = max(koff);
    ...
    %Plot koff values of endogenous and ectopic patterns, different colors
    ...
    %%%
    [x2_endo, inBetween_endo] = shadedSte(koff(endo), fluo_mean(endo), koff_ste(endo));
    colLine = 'black';
    %Convert color code to 1-by-3 RGB array (0~1 each)
    %colRGB = sscanf(strCol(2:end),'%2x%2x%2x',[1 3])/255; 
    h = fill(x2_endo, inBetween_endo, colLine);
    set(h,'facealpha',.1)
    set(h,'EdgeColor','none')
    hold on;
    plot(fluo_mean(endo), koff(endo), 'color', colLine, 'LineWidth', 2, 'LineStyle', genotypeLine);
    
    hold on
    
    %%%Plot with error shaded areas for ectopic values
    %%%
    [x2_ecto, inBetween_ecto] = shadedSte(koff(ecto), fluo_mean(ecto), koff_ste(ecto));
    colLine = 'red';
    %Convert color code to 1-by-3 RGB array (0~1 each)
    %colRGB = sscanf(strCol(2:end),'%2x%2x%2x',[1 3])/255; 
    ...
    set(h,'EdgeColor','none')
    hold on;
    plot(fluo_mean(ecto), koff(ecto), 'color', colLine, 'LineWidth', 3, 'LineStyle','-');
...
% l1 = plot(NaN,NaN, eveS1Null_eveS2wt_line, 'LineWidth', 6,  'Color', eveS1Null_eveS2Gt_line);
% l2 = plot(NaN,NaN, eveS1wt_eveS2Gt_line, 'LineWidth', 6,  'Color', eveS1wt_eveS2Gt_line);
% l3 = plot(NaN,NaN, eveS1Null_eveS2wt_line, 'LineWidth', 6,  'Color', eveS1Null_eveS2wt_line);
% l4 = plot(NaN,NaN, eveS1wt_eveS2wt_line, 'LineWidth', 6, 'Color', eveS1wt_eveS2wt_line,...
%     'MarkerEdgeColor', 'black');
% l5 = plot(NaN,NaN, '-', 'LineWidth', 3, 'Color', 'black');
% l6 = plot(NaN,NaN, '--', 'LineWidth', 3, 'Color', 'black');

%Set size of y-axis
if max(maxFreq_vec) > max(maxKoff_vec)
    axis([0 round(max(maxFluo_vec) + max(maxFluo_vec)*0.1) 0 round(max(maxFreq_vec) + max(maxFreq_vec)*0.3)]);
else
    axis([0 round(max(maxFluo_vec) + max(maxFluo_vec)*0.1) 0 round(max(maxKoff_vec) + max(maxKoff_vec)*0.3)]);
end
