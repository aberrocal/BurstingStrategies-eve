...
p1 = plot(trace_time, trace_fluoInterp, 'color', "#0072BD", 'LineWidth', 5);
%p1.Color(4) = 0.75; %add transparency to line
hold on
plot(trace_time, trace_predicted_fluo, 'color', "#D95319", 'LineWidth', 2);
hold on
...
if isequal(filename, "singleTraceFits_eveS1Null_eveS2Gt_Stripe_longform.csv")
    ...
    yyaxis right
    ylabel('cpHMM-inferred promoter state')
    ax.YTick = [];

    % plot_dummies for legend
    l1 = plot(NaN,NaN, '-', 'LineWidth', 5, 'Color', "#0072BD");
    l2 = plot(NaN,NaN, '-', 'LineWidth', 2, 'Color', "#D95319");
